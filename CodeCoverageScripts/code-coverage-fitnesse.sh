...
##### Extract CURRENT Fitnesse Test Results
    wget ${BUILD_URL}testReport/api/xml -O $FITNESSE_XML
...
##### Extract PREVIOUS Fitnesse Test Results
    wget ${JOB_URL}lastSuccessfulBuild/testReport/api/xml -O $FITNESSE_PREV_XML
...
##Get Current Total number of lines
    wget ${BUILD_URL}jacoco/api/xml -O $IT_COVERAGE_XML
...
#Get Previous Total number of lines
    wget ${JOB_URL}lastSuccessfulBuild/jacoco/api/xml -O $IT_PREV_COVERAGE_XML
