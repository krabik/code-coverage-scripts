...
## Job specific settings
    ...
    UT_COVERAGE_XML="current_coverage_cobertura.xml"
    UT_PREV_COVERAGE_XML="previous_coverage_cobertura.xml"
    COVERAGE_TOOL="cobertura"

#Clear old reports
    rm $JUNIT_XML $JUNIT_PREV_XML
##### Extract CURRENT Unit Test Results
    curl --user ykhandelwal:d281b39fd700529e65201fcab6822779 -k ${BUILD_URL}testReport/api/xml --output $JUNIT_XML
    sleep 20
...
##### Extract PREVIOUS Unit Test Results
     curl --user ykhandelwal:d281b39fd700529e65201fcab6822779 -k ${JOB_URL}lastSuccessfulBuild/testReport/api/xml --output $JUNIT_PREV_XML
    sleep 20
...
##Get Current Total number of lines
	 curl --user ykhandelwal:d281b39fd700529e65201fcab6822779 -k ${BUILD_URL}cobertura/api/xml?depth=2 --output $UT_COVERAGE_XML
	sleep 5
	 UT_CURR_LINE_TOTAL=$(cat $UT_COVERAGE_XML | tr '<' '\n' |grep Lines -B 3|grep denominator|cut -f2 -d">"|bc| awk '{printf "%.0f",$1}')
    UT_CURR_COV_TOTAL=$(cat $UT_COVERAGE_XML | tr '<' '\n' |grep Lines -A 2|grep numerator|cut -f2 -d">"|bc | awk '{printf "%.0f",$1}')
    UT_CURR_MISSED_TOTAL= $(( UT_CURR_LINE_TOTAL -  UT_CURR_COV_TOTAL))
...
#Get Previous Total number of lines
	 curl --user ykhandelwal:d281b39fd700529e65201fcab6822779 -k ${JOB_URL}lastSuccessfulBuild/cobertura/api/xml?depth=2 --output $UT_PREV_COVERAGE_XML
	sleep 5
	 UT_PREV_LINE_TOTAL=$(cat $UT_PREV_COVERAGE_XML | tr '<' '\n' |grep Lines -B 3|grep denominator|cut -f2 -d">"|bc| awk '{printf "%.0f",$1}')
    UT_PREV_COV_TOTAL=$(cat $UT_PREV_COVERAGE_XML | tr '<' '\n' |grep Lines -A 2|grep numerator|cut -f2 -d">"|bc | awk '{printf "%.0f",$1}')
    UT_PREV_MISSED_TOTAL= $(( UT_PREV_LINE_TOTAL -  UT_PREV_COV_TOTAL))
##Get Current Covered Lines & round off
...
UT_CURR_CODE_COV=0
UT_CURR_CODE_COV=$(cat $UT_COVERAGE_XML | tr '<' '\n' |grep Lines -A 4|grep ratio|cut -f2 -d">")
...
UT_PREV_CODE_COV=0
UT_PREV_CODE_COV=$(cat $UT_PREV_COVERAGE_XML | tr '<' '\n' |grep Lines -A 4|grep ratio|cut -f2 -d">")
#Limit the decimal to 5 digits (No Rounding)
...
#    #Below Baseline
#    echo "$UT_PREV_CODE_COV"
#    UT_CC_BASELINE_COMPARE_RESULT=$(echo "$UT_PREV_CODE_COV > 85.00" | bc -l)
...
#    fi
    ...
    #UT_CC_BASELINE_COMPARE_RESULT=$(echo "$UT_COVERAGE_PERCENTAGE > 85.00" | bc -l)
    if [[ $(echo "UT_CURR_CODE_COV_RND < 85" | bc) -eq 1 ]]
   then
        UT_COV_DIFF_MSG="BELOW"
    fi

#commented out for coverage check
