...
##### Extract CURRENT Unit Test Results
    curl --user download:download ${BUILD_URL}testReport/api/xml --output $JUNIT_XML
...
##### Extract PREVIOUS Unit Test Results
    curl --user download:download ${JOB_URL}lastStableBuild/testReport/api/xml --output $JUNIT_PREV_XML
...
##Get Current Total number of lines
    curl --user download:download ${BUILD_URL}jacoco/api/xml --output $UT_COVERAGE_XML
...
#Get Previous Total number of lines
    curl --user download:download ${JOB_URL}lastStableBuild/jacoco/api/xml --output $UT_PREV_COVERAGE_XML
