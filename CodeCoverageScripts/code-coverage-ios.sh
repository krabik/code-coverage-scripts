...
#=========================================================================================
#Sleep and wait for jenkins cobertura plugin to generate merged coverage report.
sleep 10
##### Variables
...
##### Extract CURRENT Unit Test Results
    curl --user download:download http://172.21.56.73:8080/job/CI_IOS/${BUILD_NUMBER}/testReport/api/xml --output $JUNIT_XML
    CURR_JUNIT_PASS=$(cat $JUNIT_XML | tr '<' '\n'|grep passCount -A1|grep passCount|cut -f2 -d">"|bc)
    CURR_JUNIT_FAIL=$(cat $JUNIT_XML | tr '<' '\n'|grep failCount -A1|grep failCount|cut -f2 -d">"|bc)
    CURR_JUNIT_SKIP=$(cat $JUNIT_XML | tr '<' '\n'|grep skipCount -A1|grep skipCount|cut -f2 -d">"|bc)
    CURR_JUNIT_TOTAL=$(echo "($CURR_JUNIT_PASS + $CURR_JUNIT_FAIL + $CURR_JUNIT_SKIP )" | bc)
##### Extract PREVIOUS Unit Test Results
    curl --user download:download http://172.21.56.73:8080/job/CI_IOS/lastStableBuild/testReport/api/xml --output $JUNIT_PREV_XML
...
##Get Current Total number of lines
	 curl --user download:download -k http://172.21.56.73:8080/job/CI_IOS/${BUILD_NUMBER}/cobertura/api/xml?depth=2 --output $UT_COVERAGE_XML
	sleep 5
...
#Get Previous Total number of lines
	 curl --user download:download -k http://172.21.56.73:8080/job/CI_IOS/lastSuccessfulBuild/cobertura/api/xml?depth=2 --output $UT_PREV_COVERAGE_XML
	sleep 5
