...
##### Extract CURRENT Unit Test Results
    curl -k ${BUILD_URL}testReport/api/xml --output $JUNIT_XML
    sleep 20
...
##### Extract PREVIOUS Unit Test Results
    curl -k ${JOB_URL}lastSuccessfulBuild/testReport/api/xml --output $JUNIT_PREV_XML
    sleep 20
...
##Get Current Total number of lines
	curl -k ${BUILD_URL}jacoco/api/xml --output $UT_COVERAGE_XML
...
#Get Previous Total number of lines
	curl -k ${JOB_URL}lastSuccessfulBuild/jacoco/api/xml --output $UT_PREV_COVERAGE_XML
